...
public class CompilePublicRFileHandler extends AbstractAndroidHandler {
    ...
    public boolean handle() {
        ...
        String javaCacheFullFilename = this.androidConfiguration.getPublicRAndroidProject().getCacheOutput() + Constants.Symbol.SLASH_LEFT + CACHE_JAVA_FILE;
        ...
        cacheOption.changedFileProcessor = new ChangedFileProcessor() {
            public boolean process(Iterable<ChangedFile> changedFileIterable) {
                if (changedFileIterable != null && changedFileIterable.iterator().hasNext()) {
                    ...
                    String publicJar = (!androidConfiguration.isApkDebug() ? androidConfiguration.getPublicRAndroidProject().getOptimizedOriginalOutput() : androidConfiguration.getPublicAndroidProject().getOptimizedProguardOutput()) + "/" + PublicAndroidProject.PUBLIC + Constants.Symbol.DOT + Constants.File.JAR;

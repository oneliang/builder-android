        ...
        for (AndroidProject androidProject : androidProjectList) {
            ...
            for (String gradleDependency : gradleDependencyList) {
                ...
                if (!dependencyFilename.endsWith(".aar")) {
                    if (dependencyFilename.endsWith(Constants.Symbol.DOT + Constants.File.JAR)) {
                ...
                final String aarBuildCacheOutput = this.androidConfiguration.getBuildOutput() + Constants.Symbol.SLASH_LEFT + "aar_build_cache";
                ...
                if (aarFileCacheMap.containsKey(aarFileFullFilename)) {
                    ...
                    aarFileUnzipOutput = aarBuildCacheOutput + Constants.Symbol.SLASH_LEFT + dependencyFilename;
                    ...
                    String aarMd5FullFilename = aarBuildCacheOutput + Constants.Symbol.SLASH_LEFT + dependencyFilename + "_md5.txt";
                    String newAarMd5 = Generator.MD5File(gradleDependency);
                    boolean aarHasChanged = false;
                    if (FileUtil.isExist(aarMd5FullFilename)) {
                        try {
                            String oldAarMd5 = new String(FileUtil.readFile(aarMd5FullFilename), Constants.Encoding.UTF8);
                    ...
                    if (aarHasChanged) {
                        try {
                            FileUtil.writeFile(aarMd5FullFilename, newAarMd5.getBytes(Constants.Encoding.UTF8));
                        } catch (Exception e) {
                            logger.error(Constants.Base.EXCEPTION, e);
                        }
                ...
                // aar res
                String aarResOutput = aarFileUnzipOutput + Constants.Symbol.SLASH_LEFT + AarProject.AAR_RES;
                androidProject.getResourceDirectoryList().add(0, aarResOutput);

                // aar libs
                String aarLibsOutput = aarFileUnzipOutput + Constants.Symbol.SLASH_LEFT + AarProject.AAR_LIBS;
                String aarJniOutput = aarFileUnzipOutput + Constants.Symbol.SLASH_LEFT + AarProject.AAR_JNI;
                androidProject.getLibsDirectoryList().add(aarLibsOutput);
                androidProject.getLibsDirectoryList().add(aarJniOutput);

                // aar assets
                String aarAssetsOutput = aarFileUnzipOutput + Constants.Symbol.SLASH_LEFT + AarProject.AAR_ASSETS;
                androidProject.getAssetsDirectoryList().add(aarAssetsOutput);

                // aar classes.jar
                String aarClassesJar = aarFileUnzipOutput + Constants.Symbol.SLASH_LEFT + AarProject.AAR_CLASSES_JAR;
                if (androidProject.getCompileClasspathList() == null) {
                    androidProject.setCompileClasspathList(new ArrayList<String>());
                }
                ...
                String aarAndroidManifest = aarFileUnzipOutput + Constants.Symbol.SLASH_LEFT + AarProject.AAR_ANDROID_MANIFEST_XML;
                androidManifestXmlList.add(aarAndroidManifest);

                // aar aidl
                String aarAidlOutput = aarFileUnzipOutput + Constants.Symbol.SLASH_LEFT + AarProject.AAR_AIDL;
